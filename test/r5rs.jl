...
sx(s) = parse(SExpression, s)

@testset "6.2.5. Numerical operations" begin
    @test evaluate(sx("(number? 10)"))
    @test !evaluate(sx("(number? \"x\")"))
    @test evaluate(sx("(complex? 3+4i)"))
    @test_broken evaluate(sx("(complex? 3)"))
    @test evaluate(sx("(real? 3)"))
    @test_broken evaluate(sx("(real? -2.5+0.0i)"))
    @test_broken evaluate(sx("(real? #e1e10)"))
    @test evaluate(sx("(rational? 6/10)"))
    @test evaluate(sx("(rational? 6/3)"))
    @test evaluate(sx("(rational? -6/3)"))
    @test evaluate(sx("(rational? 97)"))
    @test evaluate(sx("(integer? 10)"))
    @test_broken evaluate(sx("(integer? 3.0)"))
    @test evaluate(sx("(integer? 3+0i)"))
    @test !evaluate(sx("(integer? 3+1/2i)"))
    @test_broken !evaluate(sx("(integer? +i)"))
    ...
    @test evaluate(sx("(zero? -0)"))
    @test evaluate(sx("(zero? 0+0i)"))
    @test evaluate(sx("(positive? 1)"))
    @test_broken evaluate(sx("(positive? 1+0i)"))
    @test_broken !evaluate(sx("(positive? 0+0i)"))
    @test !evaluate(sx("(negative? 1)"))
    @test evaluate(sx("(negative? -1)"))
